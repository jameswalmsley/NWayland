...
{
    unsafe class LinuxUnsafeMethods
    {
        ...
        [DllImport("libc", EntryPoint = "memcpy", SetLastError = true)]
        public static extern int memset(IntPtr dest, int val, IntPtr length);
        ...
        public static extern int ftruncate(int fd, uint size);

        [DllImport("librt", EntryPoint = "shm_open", SetLastError = true)]
        public static extern int shm_open(String name, FdFlags oflag, int mode);

        [DllImport("librt", EntryPoint = "shm_unlink", SetLastError = true)]
